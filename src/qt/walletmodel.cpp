...
{
    CHODLCoinAddress addressParsed(address.toStdString());
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, std::string &termdepositquestion, const CCoinControl *coinControl)
{
    ...
    Q_FOREACH(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        {   // User-entered hodlcoin address / amount:
            ...
            if(instruction==89){
                ...
                scriptPubKey = GetTimeLockScriptForDestination(CHODLCoinAddress(rcp.address.toStdString()).Get(),chainActive.Height()+1+termDepositLength);
            }else{
                scriptPubKey = GetScriptForDestination(CHODLCoinAddress(rcp.address.toStdString()).Get());
            }
...

    {
        ...
        // reject absurdly high fee > 0.1 hodlcoin
...
{
    QString strAddress = QString::fromStdString(CHODLCoinAddress(address).ToString());
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(!out.fSpendable || !ExtractDestination(cout.tx->vout[cout.i].scriptPubKey, address))
            continue;
        mapCoins[QString::fromStdString(CHODLCoinAddress(address).ToString())].push_back(out);
    }
}
...
{
    CTxDestination dest = CHODLCoinAddress(sAddress).Get();
